...
sink(paste0("output/jobMessage/sim_vary_prop_obs_mar1_", format(Sys.time(), "%d-%b-%Y %H.%M"), ".txt"))
...
for (prop_obs_s0 in c(0.3,0.5,0.7,1)) {
  ...
  for (prop_obs_s1 in c(0.4,0.6,0.8,1)){
    ...
    write.csv(sim_result, 
              paste0("output/vary_prop/mar_1/s0-",prop_obs_s0,"_s1-",prop_obs_s1,"_seed-",sim_seed,".csv"), 
              row.names = FALSE)
