...
sink(paste0("output/jobMessage/sim_vary_prop_obs_mnar_", format(Sys.time(), "%d-%b-%Y %H.%M"), ".txt"))

for (prop_obs_s0 in c(0.3,0.5,0.7,1)) {
  
  for (prop_obs_s1 in c(0.4,0.6,0.8,1)){
    
    if(prop_obs_s0 == 1 && prop_obs_s1 == 1) next
    ...
    for(iter in 1:num_reps){
      ...
      m2.1 = rep(1,n1)
      m2.1[s1 < median(s1)] = rbinom(sum(s1 < median(s1)),1,prop_obs_s1)
      m2.0 = rep(1,n0)
      m2.0[s0 > median(s0)] = rbinom(sum(s0 > median(s0)),1,prop_obs_s0)
      df$Miss = c(m2.0, m2.1)
      df$Smiss[c(m2.0, m2.1) == 0] = NA
      
      # known MAR weights
      #w.y1 = rep(1,n1)
      #w.y1[s1 < median(s1)] = prop_obs_s1
      #w.y0 = rep(1,n0)
      #w.y0[s0 > median(s0)] = prop_obs_s0
      #df$W = c(w.y0, w.y1)
      ...
      df$W <- predict(prop_score_lm, type = "response", data = df)
    ...
    write.csv(sim_result, 
              paste0("output/vary_prop/mnar/s0-",prop_obs_s0,"_s1-",prop_obs_s1,"_seed-",sim_seed,".csv"), 
              row.names = FALSE)
