...
sink(paste0("output/jobMessage/sim_vary_prop_obs_mar2_", format(Sys.time(), "%d-%b-%Y %H.%M"), ".txt"))
...
for (prop_obs_s0 in c(0.3,0.5,0.7,1)) {
  ...
  for (prop_obs_s1 in c(0.4,0.6,0.8,1)){
    ...
    for(iter in 1:num_reps){
      ...
      m2.1 = rbinom(n1,1,prop_obs_s1)
      m2.0 = rbinom(n0,1,prop_obs_s0)
      df$Miss = c(m2.0, m2.1)
      df$Smiss[c(m2.0, m2.1) == 0] = NA
      
      # known MAR weights
      #w.y1 = rep(prop_obs_s1,n1)
      #w.y0 = rep( prop_obs_s0,n0)
      #df$W = c(w.y0, w.y1)
    ...
    write.csv(sim_result, 
              paste0("output/vary_prop/mar_2/s0-",prop_obs_s0,"_s1-",prop_obs_s1,"_seed-",sim_seed,".csv"), 
              row.names = FALSE)
