...
sink(paste0("output/jobMessage/sim_vary_prop_obs_mcar_", format(Sys.time(), "%d-%b-%Y %H.%M"), ".txt"))

for (prop_obs_s0 in c(0.3,0.5,0.7)) {
    
    prop_obs_s1 = prop_obs_s0
    ...
    write.csv(sim_result, 
              paste0("output/vary_prop/mcar/s0-",prop_obs_s0,"_s1-",prop_obs_s1,"_seed-",sim_seed,".csv"), 
              row.names = FALSE)
